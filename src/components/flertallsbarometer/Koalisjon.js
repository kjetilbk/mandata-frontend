...
const Koalisjon = ({navn, partier, farge, posisjon, support}) => {
  const partiLogoer = partier.map(parti => <Parti key={parti.forkortelse.toLowerCase()} navn={parti.navn} forkortelse={parti.forkortelse.toLowerCase()}/>);
  ...
  return (
    <div style={{float: posisjon, display: 'inline-block'}}>
      <div className={navn.toLowerCase()}>
        <h2 className={"coalition-name " +navn.toLowerCase()}>{navn}</h2>
        <div className={"partiforkortelser "+navn.toLowerCase()}>{partiForkortelser}</div>
